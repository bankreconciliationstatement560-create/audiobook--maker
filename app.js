...
function speakChunkQueue(lang, voice, rate, pitch) {
  ...
  const total = longQueue.length + 1;
  let done = 0;
  u.onstart = ()=> {
    document.getElementById('progressSection').style.display = 'block';
  };
...
// ====== DOWNLOAD (best effort, har phone pe guarantee nahi) ======
...
async function downloadAudio() {
  ...
  try {
    ...
    mediaRecorder.onstop = () => {
      ...
      a.href = url;
      a.download = 'audiobook.webm';
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      URL.revokeObjectURL(url);
      stream.getTracks().forEach(t=>t.stop());
      alert('Downloaded (agar file nahi chale to external converter se MP3 banao).');
    };
    mediaRecorder.start();
    playAudio();
    setTimeout(()=>{ if(mediaRecorder.state==='recording') mediaRecorder.stop(); },
      text.length*150);
  } catch(e){
    alert('Mic permission / browser limitation ki wajah se direct record nahi ho raha.');
  }
}
